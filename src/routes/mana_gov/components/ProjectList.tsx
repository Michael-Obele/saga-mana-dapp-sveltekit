...
const ProjectList = ({ username }: Props) => {
  ...
  return (
    <div>
      <div className="flex w-full flex-col gap-4 text-amber-300">
            ...
            <div key={proposal.id} className="flex flex-col gap-4">
              <div className="flex flex-row gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="text-sm text-amber-500">{proposal.description}</div>
                </div>
                <div className="flex flex-col gap-2">
                  <div className="text-sm text-amber-500">Budget Window: {proposal.budgetWindowLow?.toString()} - {proposal.budgetWindowHigh?.toString()}</div>
                  <div className="text-sm text-amber-500">Target Approval Date: {proposal.targetApprovalDate?.toLocaleDateString() || 'No target approval date'}</div>
                </div>
              </div>
            </div>
          ))
